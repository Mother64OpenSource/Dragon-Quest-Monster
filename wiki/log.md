...
## [2026-07-29] build | Monster size ("reborn") and synthesis-stack trait unlocking

User explained the real DQM size/reborn system with a screenshot of Slime's own real trait list (`Small Body - Critical Massacre - Zapmeister` / `Random Accelerate [P] - Sudden White Fog [H]` / `Dust of the Clan [+25] - Tactical Trooper [+50] - Strong Guard Break [+★]`): a monster's synthesis slot count ("size," 1-4) can be increased past its natural value by "reborn" once a specimen reaches +★ (a synthesis stack of +100), and doing so unlocks a further tier of traits -- P at 2+ slots, H at 3+, G at 4 -- on top of a separate ladder unlocked by the synthesis stack itself (+25/+50/+★). Both this project's own `import_traits.gd` (back when the base trait fixtures were first imported) and a stray "iirc" in the user's own explanation flagged real uncertainty here, so this entry documents exactly what got built and what's still an assumption.

**Source data, better than expected**: `import_traits.gd`'s own doc comment said this engine "has no synthesis/size/rank-offset progression system to hang [Size-tier and Rank-offset-tier traits] on yet," so only the unconditional "By Default" tier had ever been imported. Re-examining the cached `sheet1.csv` (the monster-centric sheet, still sitting in the scratchpad from that same earlier import) showed its own per-monster "Traits" cell is actually 3 newline-separated lines -- By Default / If Size [P/H/G] / If Rank Offset [+25/+50/+★] -- and, unlike the trait-centric sheet (whose own per-trait "monsters with this trait" lists are explicitly capped at "10 per category"), this cell is complete for all 803 monsters (802 have exactly 3 lines; Togrus Maximus has only 2, meaning no Size-tier data exists for it at all).

**A data point that contradicts the user's own recollection, flagged rather than silently resolved**: the user recalled "iirc, in MMK, 4 slots is still reserved to the natural 4 slots monsters" (i.e. a smaller-natured monster couldn't reborn all the way to G). But Montner -- whose own fixture has `slots: 1` (natively 1-slot) -- has a real `Quad Trouble [G]` entry in the source data. Since the sheet itself assigns a G-tier trait to a monster that isn't natively 4-slot, this was implemented per the concrete data rather than the hedged recollection: `current_size` can be set to 1-4 for any monster, with no extra species-native-size gate. If the "G reserved for natural 4-slot" rule turns out to be real, it's a validation rule to add later, not a data problem.

**Data model**: `MonsterSpecies` gained two sparse dicts -- `size_gated_trait_ids` (tier code "P"/"H"/"G" -> trait ids) and `synth_gated_trait_ids` (tier code "25"/"50"/"star" -> trait ids) -- populated by a new one-off `import_size_synth_traits.gd`, which classifies each line's entries by which tag they actually carry (P/H/G vs +25/+50/+★) rather than assuming line position, specifically so the Togrus Maximus 2-line case still parses correctly instead of misreading its Rank-offset line as a Size-tier one. 1,290 size-tier and 2,409 rank-offset trait assignments written across all 803 fixtures, 0 unmatched trait names (every parsed name slugified to an id that already exists in `database/traits_defs/`, cross-checked during import).

**Runtime model, deliberately a sandbox shortcut, not a simulated reborn ceremony**: `MonsterLoadout` gained `current_size` (0 = sentinel for "use the species' own natural size," matching the existing `nickname` sentinel convention) and `synthesis_stack` (0-100, 100 = "+★"). Real DQM reborn requires actually reaching +★ first and performing a reborn action; this project's team builder already treats skill points and skillset access as freely configurable rather than something to grind toward (see the two entries above this one), so these two values are just directly-editable numbers with a validated range (`TeamRosterManager.validate_member` flags `current_size` outside 0/1-4 or `synthesis_stack` outside 0-100) -- no ceremony simulated. New `TeamRosterManager.get_active_trait_ids(loadout, species)` folds the species' `starting_trait_ids` with whichever size/synth tiers the loadout's two values have reached; each tier check is independent (not `elif`), since both ladders are ordinal -- reaching H (3+) doesn't skip P (2+), and +★ (100) doesn't skip +50/+25. `TeamToBattleBridge.build_team()` now sources a bridged `MonsterInstance`'s `active_traits` from this function instead of raw `species.starting_trait_ids` directly (its blacksmith `TRAIT_GRANT` dedup check was updated the same way, so a crafted item can't stack with a trait a monster only has via size/synth gating either).

**UI**: `TeamMemberRow` gained two inline SpinBoxes (`SizeSpinBox` 1-4, `SynthSpinBox` 0-100) next to the existing Skills/Weapon/Blacksmith controls; editing either live-updates the row's own Traits label (which now calls `get_active_trait_ids()` instead of just `species.starting_trait_ids`, so a newly-unlocked trait actually becomes visible immediately) and persists through the normal `loadout_edited` -> save flow every other row control already uses.

**A real bug this batch's own persistence test caught**: `MonsterLoadoutLoader.to_dict()`/`load_from_dict()`/`duplicate_loadout()` build/read a hand-written Dictionary rather than relying on `@export` auto-serialization, and none of the three had been taught about the two new fields -- so `current_size`/`synthesis_stack` edits would silently vanish on save/reload (and get dropped on `duplicate_team()`) despite updating in memory just fine. Caught by a straightforward "persists to disk" check (the same shape as the existing nickname-persistence test), not a subtle one -- fixed by adding both fields to all three functions.

**Verification**: `team_roster_test_runner.gd` covers `get_active_trait_ids()`'s threshold behavior directly against Slime's real data (P/H tiers at size 2/3, both tiers active at size 3 not just the higher one, size 4 not crashing despite Slime having no G entry, the 25/50/99/100 rank-offset boundary including confirming 99 is one short of unlocking +★'s trait) plus the two new validation rules. `battle_test_runner.gd` proves the wiring reaches a real bridged `MonsterInstance` (a fresh Slime carries only its 3 starting traits; one reborn to size 3 with a maxed stack carries all 3 plus all 5 gated ones). `team_builder_ui_test_runner.gd` exercises the actual SpinBoxes, the live traits-label update, and disk persistence. All 11 headless suites re-run clean; no new `class_name` classes were added this batch, so no editor rescan was needed.

## [2026-07-29] release | v1.1 -- taunt/skill-data fixes, unlimited skill points, monster size & synthesis-stack traits

Published the second public build, bundling everything from the four entries above this one since v1.0: the Selflessness/taunt subsystem, the 9-skill v1.1 batch (Counter family, Deep Breath, Mist Me, Defending Champion -- including the real Deep Breath reset-ordering bug the new tests caught), the `total_skill_points` removal, and the monster size/synthesis-stack trait-unlocking system. Re-exported the Windows build and actually ran the fresh `.exe` before publishing (not just the headless suites) -- clean startup, no warnings, same verification discipline the v1.0 export bug established.

- Tagged `v1.1`, pushed.
- Re-zipped the build (`Dragon-Quest-Monster-Showdown-Windows-v1.1.zip`, 153MB) and published via `gh release create`.
- `gh` itself had silently dropped off this shell's PATH since the v1.0 release (present in some other shell context but not this one) -- worked around by invoking `C:\Program Files\GitHub CLI\gh.exe` directly rather than investigating the PATH mismatch further, since the binary itself was still installed and working fine.

Release: https://github.com/Mother64OpenSource/Dragon-Quest-Monster/releases/tag/v1.1

## [2026-07-29] build | Skillset slot system (Showdown-style boxes+sliders), full 384-skillset import, skillset-granted traits, Double/Triple/Quad Trouble

User reported a real bug with a screenshot: a Dragon Slime had "Tame Tiger" (20 SP) checked in its Ability Breaker panel while the cheaper Gigabreak/Magic Barrier/Abiliterator (3/6/12 SP) sat unchecked -- "completely impossible ingame," since crossing an SP threshold means a monster simply knows every move up to it, permanently, with no picking-and-choosing among already-crossed ones. Same message also reported: monsters can have infinite skill points but NOT infinite skillsets at once (3 base, +1 per slot -- 3/4/5/6 for a 1/2/3/4-slot monster), a request for a Pokemon-Showdown-style box+slider UI instead of the old "every panel visible at once with per-move checkboxes" dialog, a missing skillset (Uber Breath), skillsets that grant traits not actually granting them (Diamond Slime's own Steady Recovery/Magic Regenerator), and Double Trouble not actually granting an extra attack.

**Root cause of the ordering bug**: `MonsterLoadout.equipped_skill_ids` was a manually-toggled array, independent of `skill_point_allocation` except for an unlock-gate check -- nothing enforced that an equipped move's own threshold, and every cheaper threshold in the same panel, were also equipped. This modeled a "pick your favorite moves from what you've unlocked" mechanic that doesn't exist in the real games at all. Removed `equipped_skill_ids` entirely (species+fixtures+loader+`TeamToBattleBridge`+`team_editor_panel.gd`); a monster's known skills are now always exactly `TeamRosterManager.get_unlocked_skill_ids()`'s output -- inherently contiguous, since it's just "every threshold at or below my current SP," so the impossible state can no longer be represented at all, not just prevented by extra validation.

**Skillset slot cap**: `TeamRosterManager.validate_member()` now flags `skill_point_allocation.size() > species.slots + 2` (the same "slots + 2" figure `MonsterSpecies.slots`'s own doc comment already used for panel *count*, now actually enforced as a live cap on simultaneous panels rather than just descriptive).

**New `SkillPointDialog` UI**: completely rebuilt around `species.slots + 2` slot rows instead of showing every skillset simultaneously. An empty slot is a single "+ Add Skillset..." button that expands into a search-and-pick list (excluding skillsets already occupying another slot on the same loadout); a filled slot shows the skillset name, a remove (x) button, an `HSlider` (0 to that panel's own `max_sp()`), and a live comma-joined list of everything currently unlocked -- skills *and* traits together, matching the real "this is what you have" picture with no separate toggle anywhere. **A real UX bug caught during design, not just implementation**: `HSlider.value_changed` fires continuously while dragging (once per integer step), so the original plan of calling a full dialog `_rebuild()` from that handler would have destroyed and recreated the very slider node being dragged, breaking the gesture -- fixed by having the slider's handler update only its own value/summary labels in place, reserving full rebuilds for the discrete one-shot actions (add/remove/pick a skillset).

**Full skillset catalog, not just the curated subset**: `import_skill_panels.gd` (the original importer) only ever wrote a fixture for the 220 of 384 real skillsets that at least one monster's own curated `AvailableSkillsets` column referenced -- the other 164, including Uber Breath, were never generated as fixtures at all, so "universal skillset access" (already true since an earlier milestone) couldn't reach panels that didn't exist yet. New `import_skillsets_full.gd` regenerates all 384. The same pass also fixed a second, unrelated gap in the identical source data: a threshold whose name matched neither a known ability nor a stat-boost code was silently dropped ("passive perk/ward/trait... omitted") -- cross-referencing every real trait's own `display_name` shows every one of those dropped names is actually a real trait (Diamond Slime's own 120/150 SP rungs are Steady Recovery/Magic Regenerator -- HP/MP regen, not skills at all; Guard's own top rung turned out to be Sap Ward at 100 SP, not Mist Me at 82 as previously visible). Zero unmatched names remain across all 3,533 threshold entries once stat_boost/ability/trait are all checked. `SkillSetData`/`SkillSetLoader` gained real `"kind": "trait"` support (previously any non-"skill" kind was miscoerced into a stat_boost reading nonexistent fields) plus `unlocked_trait_ids()`/`get_trait_thresholds()` mirroring the existing skill accessors. `TeamRosterManager.get_active_trait_ids()` gained a new required `skillset_db` parameter and now folds in every allocated skillset's own unlocked traits alongside the species/size/synth ones already wired up in the previous entry.

**Double/Triple/Quad Trouble**: registered as `ExtraAttackTraitEffect` with `extra_attacks = 1/2/3` (real sourced numbers straight from their own descriptions -- "twice"/"3 times"/"4 times"). These were previously deliberately excluded (same file, same trait audit) since their real wording gates them on "when not given specific orders," an AI-vs-player-order distinction this engine has no concept of at all (every action is always a specific player order) -- explicitly overridden per direct user request, the same override Hit Squad's own placeholder number already relied on (it borrowed Double Trouble's "twice" as its closest confirmed number, back when Double Trouble itself was still unregistered).

**A real bug this batch's own verification caught, not a data problem**: after finishing the rewrite, `team_builder_ui_headless` failed at its very first row-creation check with `Trying to assign value of type 'PanelContainer' to a variable of type 'team_member_row.gd'` -- `TeamMemberRowScene.instantiate()` was returning the scene's bare root type, meaning `TeamMemberRow`'s script had failed to compile. Root cause: `team_member_row.gd`'s own `_update_traits_label()` still called `TeamRosterManager.get_active_trait_ids()` with its old 2-argument signature, missed during the signature-change sweep across the rest of the codebase. Worth noting for next time: the usual `--headless --editor --quit-after N` rescan reported this as clean -- it evidently doesn't fully compile every script's call sites the way actually loading/instantiating a scene does, so a rescan reporting no errors is not sufficient proof a cross-file signature change is safe; only an actual test run (or export) is.

**Verification**: `team_roster_test_runner.gd` gained `_check_skillset_data()` (384-count, Uber Breath resolving, Diamond Slime's exact trait thresholds at 100/120/150 SP) and a rewritten `_check_validation()` (the slot-cap flag, replacing the now-impossible-to-construct "unknown equipped skill" case). `team_builder_ui_test_runner.gd`'s allocation check was rewritten end-to-end against the real dialog: slot-count header text, the add/search/pick flow, driving a real `HSlider` and asserting the *same node instance* survives the value change (the actual proof the mid-drag-rebuild bug is fixed, not just that the allocation updated), the remove flow, the 3-slot cap leaving no Add button once full, and the picker excluding an already-used skillset. `battle_test_runner.gd`'s existing size/synth bridge check already exercises `get_active_trait_ids()`'s skillset_db parameter by extension. All 11 headless suites re-run clean.

## [2026-07-29] release | v1.2 -- skillset slot system, full skillset import, skillset-granted traits, Double/Triple/Quad Trouble

Published the third public build, bundling everything from the entry above: the equipped-skill-ordering bug fix (no more separate equip step), the Showdown-style skillset slot+slider UI, the full 384-skillset import (Uber Breath and 163 others were previously missing entirely), skillset-granted traits (Diamond Slime's Steady Recovery/Magic Regenerator), and Double/Triple/Quad Trouble. Re-exported the Windows build and actually ran the fresh `.exe` before publishing -- clean startup, no warnings.

- Tagged `v1.2`, pushed.
- Zipped the build (`Dragon-Quest-Monster-Showdown-Windows-v1.2.zip`, 153MB) and published via `gh release create` (using the full `C:\Program Files\GitHub CLI\gh.exe` path directly, same PATH workaround as the v1.1 release).

Release: https://github.com/Mother64OpenSource/Dragon-Quest-Monster/releases/tag/v1.2
