...
func setup(p_roster: TeamRosterManager, p_monster_db: MonsterDatabase, p_skill_db: SkillDatabase, p_skillset_db: SkillSetDatabase, p_trait_db: TraitDatabase, p_weapon_db: WeaponDatabase, p_blacksmith_db: BlacksmithDatabase) -> void:
	roster = p_roster
...
func _add_row(parent: HBoxContainer, loadout: MonsterLoadout) -> void:
	...
	row.setup(loadout, real_index, monster_db, skill_db, skillset_db, trait_db, weapon_db, blacksmith_db)
...
func _update_validation_banner() -> void:
	...
	for member in current_team.members:
		if not roster.validate_member(member, monster_db, skillset_db, weapon_db, blacksmith_db).is_empty():
			issue_count += 1
